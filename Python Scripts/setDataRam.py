...
with open('Ryans-Final\\ryans-desktop-ram.csv', 'r', newline='',encoding="utf-8",errors='ignore') as rf:
        ...
        for i in range(0, len(ryan_df)):
            if not final_names.__contains__(row[0]):
                if ryan_df.at[i, 'name'][0:10] == row[0][0:10]:
                    ...
                    capacity = str(ryan_df.at[i, 'capacity'])

                    df.loc[j] = [brand] + [row[0]] + [ryan_price] + [str(row[2]).replace(',','')]+[row[1]]+[ryans_link]+[pic_link]+[description]+[capacity]
                    final_names.append(row[0])
                    j = j + 1
    print()
export_csv = df.to_csv(r'ram_final.csv', index=None, header=True)
